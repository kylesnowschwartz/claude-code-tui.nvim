...
T["Real Data Integration"]["result_node_content_popups"] = function()
    ...
    if result.result_count > 0 then
        ...
        if result.sample_node then
            ...
            Helpers.expect.truthy(result.sample_node.has_structured_content, "Result nodes should have structured_content")
        end
    end
end
...
T["Real Data Integration"]["content_renderer_with_real_data"] = function()
    ...
    if result.found_result_node then
        Helpers.expect.truthy(result.render_success, "Should successfully render real result content: " .. tostring(result.render_error))
    end
end
